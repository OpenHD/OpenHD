...
public:
    ...
    * Do nothing if such an instance is not found.
    ...
    [[nodiscard]] const std::list<std::unique_ptr<UDPForwarder>> &getForwarders() const;
private:
...
public:
     ...
     */
    explicit UDPReceiver(std::string client_addr, int client_udp_port, OUTPUT_DATA_CALLBACK cb);
    ~UDPReceiver();
    void loopUntilError();
    ...
    void forwardPacketViaUDP(const std::string& destIp,int destPort,const uint8_t *packet, std::size_t packetSize) const;
    void stopLooping();
