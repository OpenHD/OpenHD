 ...
 */
std::vector<std::string> getAllEntriesFullPathInDirectory(const std::string& directory);

// Same as above, but returns the filenames only.
std::vector<std::string> getAllEntriesFilenameOnlyInDirectory(const std::string& directory);
...
std::optional<std::string> opt_read_file(const std::string& filename,bool log_debug=true);
