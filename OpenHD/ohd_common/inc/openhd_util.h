 ...
 * @param print_debug print the command executed, this can be usefully for debugging -to replicate, just copy the command from the log message
 ...
 * Not sure how to describe this - it runs a command and returns its shell output. NOTE: This just returns the shell output, it does not check if the executed command is actually available on the system. If the command is not available on the system, it most likely returns "command not found" as a string.
 * @param command the command and its args to run
 * @return the shell output, or std::nullopt if something went wrong.
 */
std::optional<std::string> run_command_out(const std::string& command,bool debug = false);
...
std::string string_in_between(const std::string& start,
                                     const std::string& end,
                                     const std::string& value,
                                     bool debug=false);

/**
 * All these string_to_xxx methods are guaranteed to never throw an exception and
 * return std::nullopt in case parsing fails.
 */
