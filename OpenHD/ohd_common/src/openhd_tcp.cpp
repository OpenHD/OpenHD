...
openhd::TCPServer::TCPServer(const std::string tag,openhd::TCPServer::Config config,bool debug)
    : m_config(config),m_debug(debug){
  m_console = openhd::log::create_or_get(tag);
  assert(m_console);
  m_accept_thread=std::make_unique<std::thread>(&TCPServer::loop_accept, this);
