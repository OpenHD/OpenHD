...
class OHDProfile {
 public:
  ...
  is_air(is_air),started_from_service(started_from_service1),unit_id(std::move(unit_id1)){};
  ...
  const bool started_from_service;
  ...
  [[nodiscard]] std::string to_string()const{
	...
	ss<<"OHDProfile{"<<(is_air ? "Air":"Ground")<<":"<<unit_id<<
            " started_from_service:"<<OHDUtil::yes_or_no(started_from_service)<<"}";
	return ss.str();
  }
};
...
static std::shared_ptr<OHDProfile>  discover(bool is_air,bool started_from_service) {
  ...
  auto ret=std::make_shared<OHDProfile>(is_air,started_from_service,unit_id);
  return ret;
}
