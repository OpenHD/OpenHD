...
MicrohardLink::MicrohardLink(OHDProfile profile) : m_profile(profile) {
  ...
  if (m_profile.is_air) {
    ...
    auto cb_video_rx = [this](const uint8_t* payload, std::size_t payloadSize) {
      m_rx_total_bytes.fetch_add(static_cast<uint64_t>(payloadSize),
                                 std::memory_order_relaxed);
      m_rx_total_packets.fetch_add(1, std::memory_order_relaxed);
      m_last_rx_packet_ts_ms.store(openhd::util::steady_clock_time_epoch_ms(),
                                   std::memory_order_relaxed);
...
MicrohardLink::~MicrohardLink() {
  stop_stats_thread();
  if (m_video_rx) m_video_rx->stopBackground();
  if (m_telemetry_tx_rx) m_telemetry_tx_rx->stopBackground();
}
...
void MicrohardLink::start_stats_thread() {
...
void MicrohardLink::stop_stats_thread() {
...
void MicrohardLink::update_link_stats() {
