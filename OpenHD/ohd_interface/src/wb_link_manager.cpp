...
static std::vector<uint8_t> pack_management_frame(const DataManagementTxBandwidth& data) {
...
static std::vector<uint8_t> pack_management_frame(const DataManagementSensitivityStatus& data) {
    std::vector<uint8_t> ret;
    ret.resize(1+sizeof(data));
...
static std::string management_frame_to_string(const DataManagementTxBandwidth &data) {
...
m_last_channel_width_change_timestamp_ms{OHDUtil::steady_clock_time_epoch_ms()}
{
    ...
    auto mgmt_handler=std::make_shared<WBTxRx::StreamRxHandler>(MANAGEMENT_RADIO_PORT_GND_TX,cb_packet, nullptr);
...
void ManagementAir::loop() {
    while (m_tx_thread_run){
        ...
        DataManagementTxBandwidth managementFrame{m_curr_frequency_mhz.load(), m_curr_channel_width_mhz.load()};
