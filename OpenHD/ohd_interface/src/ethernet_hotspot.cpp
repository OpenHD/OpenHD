...
EthernetHotspot::~EthernetHotspot() { disable(); }
...
void EthernetHotspot::enable() {
  ...
  m_check_connection_thread =
      std::make_unique<std::thread>([this]() { loop_infinite(); });
...
void EthernetHotspot::discover_device_once() {
  ...
  const auto run_command_result_opt = OHDUtil::run_command_out(
      fmt::format("arp -an -i {} | grep -v incomplete", m_device));
  ...
  const auto ip_external_device =
      openhd::ethernet::get_ip_address_in_between_brackets(run_command_result,
                                                           true);
  if (!OHDUtil::is_valid_ip(ip_external_device)) {
    m_console->warn("[{}] is not a valid ip", ip_external_device);
    return;
  }
  // When we reach here we have a valid ip of the device connected - now check
  // if it disconnects
  const auto external_device =
      openhd::ExternalDevice{"ETH0", ip_external_device};
  ...
  while (!m_check_connection_thread_stop) {
    std::this_thread::sleep_for(std::chrono::seconds(1));
    const auto tmp = OHDUtil::run_command_out(
        fmt::format("arp -an -i {} | grep -v incomplete", m_device));
