...
static std::string create_command_wifi_client(const std::string ssid,
                                              const std::string pw) {
  return fmt::format("sudo nmcli dev wifi connect \"{}\" password \"{}\"",ssid,pw);
}
...
bool WiFiClient::create_if_enabled() {
  ...
  const auto command= create_command_wifi_client(config.WIFI_LOCAL_NETWORK_SSID,config.WIFI_LOCAL_NETWORK_PASSWORD);
