...
* `--dual` enables a loopback test mode that periodically sends MAVLink ping packets and tracks how many are received back on the
...
| `t` | Immediately send a random OpenHD telemetry message (or a dual-mode ping when `--dual` is active) |
| `q` | Quit the debugger |

The status line at the top reports the outcome of the most recent command and whether logging is enabled. When `--dual` is active,
