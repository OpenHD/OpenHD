    ...
    : CameraStream(platform,std::move(camera_holder),std::move(out_cb))
{
  ...
  if(m_camera_holder->get_camera().is_camera_type_usb_infiray()){
      openhd::set_infiray_custom_control_zoom_absolute_async(m_camera_holder->get_settings().infiray_custom_control_zoom_absolute_colorpalete);
  }
...
std::string GStreamerStream::create_source_encode_pipeline(const CameraHolder &cam_holder) {
    ...
    if(camera.requires_rpi_mmal_pipeline()){
      ...
      if(camera.camera_type==X_CAM_TYPE_ROCK_HDMI_IN){
        ...
        pipeline<<OHDGstHelper::createDummyStreamX(OHDPlatform::instance(),setting);
...
void GStreamerStream::handle_change_bitrate_request(openhd::LinkActionHandler::LinkBitrateInformation lb) {
...
void GStreamerStream::stream_once() {
  ...
  openhd::LinkActionHandler::instance().set_cam_info_status(m_camera_holder->get_camera().index, CAM_STATUS_RESTARTING);
  setup();
  start();
  ...
  openhd::LinkActionHandler::instance().set_cam_info_status(m_camera_holder->get_camera().index, CAM_STATUS_STREAMING);
  //
    ...
    if(currently_applied_bitrate != m_curr_dynamic_bitrate_kbits){
      ...
      if(m_bitrate_ctrl_element!=std::nullopt){
        ...
        if(change_bitrate(bitrate_ctrl_element,hacked_bitrate_kbits)){
          ...
          openhd::LinkActionHandler::instance().set_cam_info_bitrate(m_camera_holder->get_camera().index, currently_applied_bitrate);
        }else{
