...
struct XCamera {
  ...
  bool requires_orqa_pipeline() const {
    return camera_type >= 122 && camera_type <= 125;
  }
  ...
  bool requires_rockchip1126_mpp_csi_pipeline() const { return camera_type == 141; }
  bool requires_rockchip1126_mpp_testsrc_pipeline() const { return camera_type == 141; }
...
static bool is_orqa_csi_camera(int cam_type) {
  return cam_type >= 122 && cam_type <= 125;
}
