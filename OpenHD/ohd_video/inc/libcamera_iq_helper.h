...
// - openhd strictly doesn't use floats, but easier to understand values instead.
// or invalid.
namespace openhd::libcamera{

// Specify a fixed brightness parameter. Positive values (up to 1.0) produce brighter images;
...
static std::optional<float> get_brightness(const CameraSettings& settings){
  if(!openhd::validate_openhd_brightness(settings.openhd_brightness))return std::nullopt;
  if(settings.openhd_brightness==OPENHD_BRIGHTNESS_DEFAULT)return std::nullopt;
...
// Specify a fixed contrast parameter. Normal contrast is given by the value 1.0;
// larger values produce images with more contrast.
static std::optional<float> get_contrast(const CameraSettings& settings){
  if(!openhd::validate_openhd_contrast(settings.openhd_contrast))return std::nullopt;
...
// Specify a fixed saturation parameter. Normal saturation is given by the value 1.0;
// larger values produce more saturated colours; 0.0 produces a greyscale image.
static std::optional<float> get_saturation(const CameraSettings& settings){
  if(!openhd::validate_openhd_saturation(settings.openhd_saturation))return std::nullopt;
  if(settings.openhd_saturation==OPENHD_SATURATION_DEFAULT)return std::nullopt;
  return remap_libcamera_openhd_int_to_libcamera_float(
      settings.openhd_saturation);
}

// A value of 0.0 means no sharpening. The minimum value means minimal sharpening, and shall be 0.0 unless the camera
// can't disable sharpening completely. The default value shall give a "reasonable" level of sharpening,
// suitable for most use cases. The maximum value may apply extremely high levels of sharpening,
