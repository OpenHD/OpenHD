...
def render_header(
    ...
    lines.append("")
    lines.append("namespace openhd::camera_registry {")
    lines.append("")
    for entry in camera_types:
        key = entry["key"]
        cid = entry["id"]
    ...
    lines.append("};")
    lines.append("")
    lines.append("inline constexpr std::array kCameraTypeNameEntries = {")
    for entry in camera_types:
        name = escape_cpp_string(entry["name"])
        cid = entry["id"]
        lines.append(f'  CameraTypeNameEntry{{{cid}, "{name}"}},')
    lines.append("};")
    lines.append("")
    ...
    lines.append("    }")
    lines.append("  }")
    lines.append('  return std::string{"UNKNOWN ("} + std::to_string(camera_type) + ")";  // NOSONAR')
    lines.append("}")
    lines.append("")
    ...
    for entry in camera_types:
        ...
        if not resolutions:
            continue
        key = entry["key"]
        lines.append(f"inline constexpr std::array kCameraResolutions_{key} = {{")
        for resolution in resolutions:
            width = resolution["width"]
            height = resolution["height"]
            fps = resolution["fps"]
            lines.append(f"  ResolutionEntry{{{width}, {height}, {fps}}},")
        lines.append("};")
        lines.append("")

    lines.append("inline constexpr std::array kCameraResolutionEntries = {")
    for entry in camera_types:
        resolutions = entry.get("resolutions")
        if not resolutions:
            continue
        key = entry["key"]
        lines.append(
            f"  CameraResolutionEntry{{{key}, kCameraResolutions_{key}.data(), "
            f"kCameraResolutions_{key}.size()}},"
        )
    lines.append("};")
    lines.append("")
    ...
    for entry in manufacturer_groups:
        ...
        lines.append(f"inline constexpr std::array kManufacturer_{group_symbol}_Cameras = {{")
        for cam_key in cameras:
            cam = camera_by_key[cam_key]
            label = escape_cpp_string(cam["ui_label"])
            lines.append(f"  CameraUiEntry{{{cam_key}, \"{label}\"}},")
        lines.append("};")
        lines.append("")
        group_name = escape_cpp_string(entry["name"])
        lines.append(
            f"inline constexpr ManufacturerEntry kManufacturer_{group_symbol} = "
            f"{{\"{group_name}\", kManufacturer_{group_symbol}_Cameras.data(), "
            f"kManufacturer_{group_symbol}_Cameras.size()}};"
        )
        lines.append("")
    ...
    for entry in platform_choices:
        ...
        lines.append(f"inline constexpr std::array kPlatform_{symbol}_Manufacturers = {{")
        for group_key in manufacturers:
            group_symbol = escape_cpp_string(group_key).upper()
    ...
    lines.append("inline constexpr std::array kPlatformCameraChoices = {")
    for entry in platform_choices:
        platform = entry["platform"]
        symbol = escape_cpp_string(platform).upper()
        lines.append(
            f"  PlatformCameraChoicesEntry{{{platform}, kPlatform_{symbol}_Manufacturers.data(), "
            f"kPlatform_{symbol}_Manufacturers.size()}},"
        )
    lines.append("};")
    lines.append("")

    if secondary_choices:
        lines.append("inline constexpr std::array kSecondaryManufacturers = {")
        for group_key in secondary_choices:
            group_symbol = escape_cpp_string(group_key).upper()
            lines.append(f"  kManufacturer_{group_symbol},")
        lines.append("};")
        ...
        lines.append("inline constexpr std::array<ManufacturerEntry, 0> kSecondaryManufacturers = {};")
        lines.append("inline constexpr std::size_t kSecondaryManufacturerCount = 0;")
        lines.append("")

    if fallback_choices:
        lines.append("inline constexpr std::array kFallbackManufacturers = {")
        for group_key in fallback_choices:
            group_symbol = escape_cpp_string(group_key).upper()
            lines.append(f"  kManufacturer_{group_symbol},")
        lines.append("};")
        ...
        lines.append("inline constexpr std::array<ManufacturerEntry, 0> kFallbackManufacturers = {};")
