...
void AirTelemetry::setup_openhd_uart_telemetry() {
  ...
  m_openhd_uart_serial->configure(
      ...
      [this](const std::vector<MavlinkMessage> messages) {
        ...
        // Treat UART originated messages as if they were generated by this
        // OpenHD instance so that external devices can masquerade as the local
        // system/component when interacting over MAVLink.
        for (auto& message : forwarded) {
          m_console->info(
              "Rewriting UART message id={} to sys={} comp={} for forwarding",
              message.m.msgid, _sys_id, MAV_COMP_ID_ONBOARD_COMPUTER);
          message.m.sysid = _sys_id;
          message.m.compid = MAV_COMP_ID_ONBOARD_COMPUTER;
        }
        m_console->info(
            "Forwarding {} MAVLink message(s) from OpenHD UART telemetry to ground",
            forwarded.size());
