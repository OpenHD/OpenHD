...
class MavlinkMessageHandler {
 public:
  ...
  void register_one(uint16_t msg_id, const Callback& callback,
                    const void* cookie);
  void register_one(uint16_t msg_id, std::optional<uint8_t> cmp_id,
                    const Callback& callback, const void* cookie);
