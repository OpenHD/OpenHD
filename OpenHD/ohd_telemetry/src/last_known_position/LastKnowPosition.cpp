...
static std::string get_this_flight_filename(){
  ...
  std::stringstream ss;
  ss<<LAST_KNOWN_POSITION_DIRECTORY<<std::put_time(&tm, "%d-%m-%Y_%H-%M-%S")<<".txt";
  return ss.str();
}
...
void LastKnowPosition::on_new_position(double latitude, double longitude,
  ...
  threadsafe_update_position(LastKnowPosition::Position{std::chrono::steady_clock::now(),latitude,longitude,altitude});
}

void LastKnowPosition::write_position_loop() {
    while (m_write_run){
        std::this_thread::sleep_for(std::chrono::seconds(1));
        // Get the last X positions (if there is no update,aka no new data or crash, we get nullopt)
...
std::optional<std::list<LastKnowPosition::Position>> LastKnowPosition::threadsafe_get_positions_if_updated() {
...
void LastKnowPosition::threadsafe_update_position(LastKnowPosition::Position position) {
    std::lock_guard<std::mutex> guard(m_position_mutex);
...
std::string LastKnowPosition::positions_to_file(const std::list<Position> &positions) {
    std::stringstream ss;
    for(const auto& position:positions){
        ss<<fmt::format("Lat:{},Lon:{},Alt:{}\n",position.latitude,position.longitude,position.altitude_m);
    }
    return ss.str();
}
