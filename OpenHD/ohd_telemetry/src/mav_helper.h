...
static MavlinkMessage createLog(
    const int sys_id, const int comp_id, std::string tag, std::string message,
    uint8_t severity = 0,
    uint64_t timestamp =
  ...
  mavlink_msg_openhd_log_message_pack(sys_id, comp_id, &msg.m, severity,
                                      (const char*)tag.c_str(),
                                      (const char*)message.c_str(), timestamp);
  return msg;
}
