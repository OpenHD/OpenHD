      ...
      while ((pos = buffer.find('\n')) != std::string::npos) {
        ...
        if (parts.size() >= 22 && parts[0] == "MSG") {
          ...
          if (msg_type == "1") {
            ...
            if (alt_opt) {
              v.altitude = alt_opt.value() * 304.8; // feet to mm
              v.flags |= ADSB_FLAGS_VALID_ALTITUDE;
            }
            if (lat_opt) {
              v.lat = lat_opt.value() * 1e7;
              v.flags |= ADSB_FLAGS_VALID_COORDS;
            }
            if (lon_opt) {
              v.lon = lon_opt.value() * 1e7;
              v.flags |= ADSB_FLAGS_VALID_COORDS;
            }
          } else if (msg_type == "4") {
            ...
            if (speed_opt) {
              v.hor_velocity = speed_opt.value() * 51.4444; // knots to cm/s
              v.flags |= ADSB_FLAGS_VALID_VELOCITY;
            }
            if (track_opt) {
              v.heading = track_opt.value() * 100.0; // deg to cdeg
              v.flags |= ADSB_FLAGS_VALID_HEADING;
            }
            if (vert_rate_opt) {
              v.ver_velocity = vert_rate_opt.value() * 0.508; // ft/min to cm/s
...
std::vector<MavlinkMessage> AdsbComponent::generate_mavlink_messages() {
  ...
  // optionally, clear older vehicles here, but for now we just keep returning them
  // until memory is bounded or timeout mechanism added if required.
  // Actually let's clear them after sending to avoid a massive list of old planes.
  m_adsb_vehicles.clear();
