...
void OnboardComputerStatusProvider::calculate_other_until_terminate() {
  while (!terminate){
	...
	const int curr_memory_usage_perc=static_cast<int>(OnboardComputerStatus::calculate_memory_usage_percent());
    ...
    }
    {
	  ...
	  m_curr_onboard_computer_status.ram_usage=curr_memory_usage_perc;
    }
