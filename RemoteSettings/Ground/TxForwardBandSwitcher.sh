...
if [ "$PrimaryCardMAC" == "0" ]; then
	...
	/usr/local/bin/wfb_tx -k 1 -n 1 -u 4321 -t 2 -p 42 -B 20 -M 0 -K /tmp/tx.key $NICS_LIST
else
	while true
	do
...
#    		if [ "$EncryptionOrRange" == "Range" ]; then
			/usr/local/bin/wfb_tx -k 1 -n 1 -u 4321 -t 2 -p 42 -B 20 -M 0 -K /tmp/tx.key $PrimaryCardMAC
#		else
